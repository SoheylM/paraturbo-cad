...
file = open('best_solution_Element_2.pickle','rb')
...
#tip radius (7mm-35mm)
r_4 = round(Element['r4_COMP1'][0],12)*1000

#tip width (0.1mm-10.5mm)
b_4 = (Element['b4_r4_COMP1'][0])*r_4

#inlet hub radius (0.7mm-10.5mm)
r_2h = (Element['r2h_r4_COMP1'][0])*r_4

#inlet shoulder radius (0.84mm-24.5mm)
r_2s = (Element['r2s_r2h_COMP1'][0])*r_2h

#inducer inlet radius (0.84mm-35mm)
r_1 = (Element['r1_r2s_COMP1'][0])*r_2s

# diffuser exit radius (7mm-52.5mm)
r_5 = (Element['r5_r4_COMP1'][0])*r_4

#blade thickness (0.1mm-0.5mm)
e_bld = (Element['Blade_e_COMP1'][0])*1000

#inducer length (7mm-140mm)
L_ind = (Element['L_ind_r4_COMP1'][0])*r_4

#tip clearance (0.001mm-0.158mm)
e_tip = (Element['Clearance_b4_COMP1'][0])*b_4

#backface clearance (0.007mm-5.25mm)
e_back = (Element['Backface_r4_COMP1'][0])*r_4

#exit blade angle (-45deg-0deg)
beta_4 = Element['beta4_COMP1'][0]

if beta_4 ==0:
    beta_4 = 0.01

#number of blades (5blades-11blades)
N_bld = int(Element['N_blades_COMP1'][0])

# radius of rotor
R_rot = (Element['R_ROT'][0])*1000
...
#         if i in aux.keys():
#             show_object(aux[i],options={"alpha":0, "color": (255, 0, 0)})

# #modeling every element only if the inner diameter DI is not equal to the outer diameter DA
#     if DI2[i]!=DA2[i]:

#         rotor[i] = (cq.Workplane("XY")
...
# #displaying every element
#         if i in rotor.keys():
#             show_object(rotor[i],options={"alpha":0, "color": (0, 255, 0)})

# #modeling every element only if the inner diameter DI is not equal to the outer diameter DA
#     if DI3[i]!=DA3[i]:

#         impeller[i] = (cq.Workplane("XY")
...
# #displaying every element
#         if i in impeller.keys():
#             show_object(impeller[i],options={"alpha":0, "color": (0, 0, 255)})
